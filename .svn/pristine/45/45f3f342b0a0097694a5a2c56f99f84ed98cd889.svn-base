...
<!-- **********************************************************************************************************************************************************
   ...
   if(sessionVal == ""){
    ...
    <section id="main-content">
      <section class="wrapper">
        <h3><img src="${path}/a00_Common/a01_css/schedule.png" alt="" style="width:50px; height:50px"> |  일정 관리</h3>
        <div class="row mt">
          <div class="col-lg-12">
            <div class="content-panel">
              <form id="frm01" class="form"  method="post">
		  	  <nav class="navbar navbar-expand-sm bg-dark navbar-dark" >
		  	  <div class="search_prj">
		  	  <label class="col-lg-2 control-label">프로젝트번호</label>
			  <input name="prjNo" value="${param.prjNo}" id="input01" class="form-control mr-sm-2" placeholder="프로젝트번호 입력" style="width:200px;"/>
			  <label class="col-lg-2 control-label">프로젝트명</label>
			  <input name="prjName" value="${param.prjName}"  id="input01" class="form-control mr-sm-2" placeholder="프로젝트명 입력" style="width:200px;"/>
			  </div>
			 <!--  
              ...
              <section id="unseen">
                <table class="table table-bordered table-striped table-condensed">
                  <thead>
                    <tr>
                      <th width="7%">프로젝트 번호</th>
                      <th width="13%" class="numeric">프로젝트명</th>                      
                      <th width="22%" class="numeric">달성률<img src="${path}/a00_Common/a01_css/flag.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                      <th width="5%" class="numeric">상태<img src="${path}/a00_Common/a01_css/setting.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                      <th width="5%" class="numeric">중요<img src="${path}/a00_Common/a01_css/pin.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                      <th width="13%" class="numeric">시작일<img src="${path}/a00_Common/a01_css/clock.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                      <th width="13%" class="numeric">종료일<img src="${path}/a00_Common/a01_css/clock.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                      <th width="6%" class="numeric">등록<img src="${path}/a00_Common/a01_css/calendar.png" alt="" style="width:20px; height:20px; margin-left:10px;"></th>
                    </tr>
                  </thead>
                  <tbody>
                  <c:forEach var="prj" items="${prjList}">
                    <tr id="prj_tr" ondblclick="goTodoList('${prj.prjNo}')">
                      <td >${prj.prjNo}</td>
                      <td class="numeric">${prj.prjName}</td>
                      <td>
                      <div class="progress progress-striped active">
                      <c:if test="${prj.prjRate <= 30}">                     
		                <div class="progress-bar progress-bar-danger" role="progressbar" aria-valuenow="${prj.prjRate}" aria-valuemin="0" aria-valuemax="100" style="width: ${prj.prjRate}%">
		                  <span class="sr-only">${prj.prjRate}% Complete</span>
		                </div>		              
		              </c:if>
		              <c:if test="${prj.prjRate > 30 && prj.prjRate <= 50}">
		                <div class="progress-bar progress-bar-warning" role="progressbar" aria-valuenow="${prj.prjRate}" aria-valuemin="0" aria-valuemax="100" style="width: ${prj.prjRate}%">
		                  <span class="sr-only">${prj.prjRate}% Complete</span>
		                </div>
		              </c:if>
		              <c:if test="${prj.prjRate > 50 && prj.prjRate < 80}">
		                <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="${prj.prjRate}" aria-valuemin="0" aria-valuemax="100" style="width: ${prj.prjRate}%">
		                  <span class="sr-only">${prj.prjRate}% Complete</span>
		                </div>
		              </c:if>
		              <c:if test="${prj.prjRate >= 80}">
		                <div class="progress-bar progress-bar" role="progressbar" aria-valuenow="${prj.prjRate}" aria-valuemin="0" aria-valuemax="100" style="width: ${prj.prjRate}%">
		                  <span class="sr-only">${prj.prjRate}% Complete</span>
		                </div>
		              </c:if>
		              </div>
              		  ...
              		  <td class="numeric">
              		  <c:if test="${prj.prjRate >= 50 && prj.prjRate < 100}">
              		  <span class="label label-success label-mini">done</span>
              		  </c:if>
              		  <c:if test="${prj.prjRate < 50}">
              		  <span class="label label-warning label-mini">Due</span>
              		  </c:if>
              		  <c:if test="${prj.prjRate == 100}">
                      ...
                      <td class="numeric">
                      	<button class="btn btn-primary btn-xs" onclick="goCalendar('${prj.prjNo}')"
  ...
  <script type="text/javascript">
  	function goTodoList(prjNo) {
  		if (confirm("to-do list로 이동하시겠습니까?")) {
  			location.href="${path}/todoList.do?prjNo="+prjNo;
  		}
  	}
