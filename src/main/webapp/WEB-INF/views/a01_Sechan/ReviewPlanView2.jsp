   ...
   if(sessionVal == ""){
    ...
    <section id="main-content">
      <section class="wrapper">
        <h3><i class="fa fa-angle-right"></i> 검토계획 조회</h3>
        ...
        <div class="row mb">
          ...
          <div class="content-panel">
              ...
              <table cellpadding="0" cellspacing="0" border="0" class="display table table-bordered" id="hidden-table-info">
                <thead>
                  <tr>
                    <th>검토계획ID</th>
                    <th>검토계획명</th>
                    <th class="hidden-phone">프로젝트명</th>
                    <th class="hidden-phone">검토계획기간</th>
                    <th class="hidden-phone">승인상태</th>
                    <th class="hidden-phone">승인허가/반려</th>
                    <th class="hidden-phone">위험 관리</th>
                ...
                <tbody>
                  ...
                  <c:forEach var="bd" items="${rlist}">
		       		...
		       		<form id="frm01" action="${path}/updateRplanStatus.do" class="form" >  
		       			  <tr ondblclick="gorDetail('${bd.rplanNo}')"><td>${bd.rplanNo} </td><td>${bd.rplanName}</td>
		       			  	<td>${bd.prjName}</td>
		       			  	<td><fmt:formatDate value="${bd.requestDate}"/>~<fmt:formatDate value="${bd.approvalDate}"/></td>
		       				<td>${bd.rplanStatus}</td>
		       				
		       				<td><button class="btn btn-success btn-xs" onclick="commitStatus('${bd.rplanNo}','${bd.prjNo}')"><i class="fa fa-check"></i></button>
		                      <button class="btn btn-danger btn-xs" onclick="deleteColumn('${bd.rplanNo}','${bd.prjNo}')"><i class="fa fa-trash-o " ></i></button>
		                  	 
		                    </td>
							<td>
								<button class="btn btn-danger btn-xs" onclick="goRisk('${bd.rplanNo}','${bd.prjNo}')"><i class="fa fa-check" ></i></button>
            ...
            <script type="text/javascript">
           		function gorDetail(rplanno){
					location.href="${path}/ReviewPlanDetail.do?rplanno="+rplanno;           			
           		}
           		

           		function commitStatus(rplanno,prjno){
           		 	var auth = "${emp.auth}"
           		 if (auth=="임원") {
           			if(confirm("승인하시겠습니까?")){
           				
           				
           				location.href="${path}/updateRplanStatus.do?rplanNo="+rplanno;
           				alert("승인완료!")
           		...
           		function deleteColumn(rplanno,prjno){
           		 var auth = "${emp.auth}"
           		 if (auth=="임원") {
           			if(confirm("반려하시겠습니까?")){
           				
           				
           				location.href="${path}/updateRplanStatusRJ.do?rplanNo="+rplanno;
           				alert("반려완료")
           				location.href="${path}/rplanlist.do?prjNo="+prjno;
           		...
           		function goRisk(rplanno,prjno){
           			location.href="${path}/risk.do?rplanno="+rplanno+"&prjNo="+prjno;
           		}
          ...
          <!-- page end-->
          	  <br>
          	  <p>               
                <button  type="button" class="btn btn-theme button" onclick="goInsertRplan('${bd.prjNo}')" >검토계획 등록</button>
              </p>
        </div>
        <script type="text/javascript">
        	function goInsertRplan(prjno){
        		location.href="${path}/rplanInsertForm.do?prjNo="+'${param.prjNo}'
        	}
